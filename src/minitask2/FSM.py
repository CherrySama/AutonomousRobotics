...
class FSM():
    ...
    def show_right_range(self):
        ...
        if self.range:
            ...
            # 如果满足距离条件的点数超过一定比例，则认为有墙
            # 超过80%的点在距离范围内
            rospy.loginfo(f"Right in area:{len(right_distances)}, 60%:{len(self.right_ranges) * 0.6}")
            rospy.loginfo(f"{len(right_distances)/len(self.right_ranges)*100}% in area")
    ...
    #         if self.Wall_detect():
    #             if 0.0 < abs(self.range[275] - self.range[265]) < 0.005:
    #             # 根据是否检测到墙来决定回到哪个状态
    #                 self.Stop()
    #                 self.state = "FOLLOW_WALL"
    #         else:
    ...
    #     if not self.Obstacle_detect() and 0.0 < abs(self.range[275] - self.range[265]) < 0.005:
    #         # 根据是否检测到墙来决定回到哪个状态
    #         # rospy.loginfo(f"change:{self.get_right_percent()}")
    #         if self.Wall_detect():
    #             self.Stop()
    #             self.state = "FOLLOW_WALL"
    #             return
    #         # 仍需要继续旋转至平行与墙面
    #         elif self.get_right_percent() >= 0.3 and self.need_turn_left:
    #             while self.Wall_detect() == False and self.get_right_percent() >= 0.3 and self.need_turn_left and abs(self.range[275] - self.range[265]) < 0.005:
    #                 # rospy.loginfo(f"log{self.get_right_percent()}")
    #                 self.pub.publish(self.avoid_obstacle)
    #                 self.r.sleep()
    #             self.Stop()
    #             if self.Wall_detect():
