...
class SinglePointMover:
    ...
    def send_goal(self, x, y, theta):
        ...
        if success and actionlib.GoalStatus.SUCCEEDED:
            rospy.loginfo(f"Reached target point (x={x}, y={y}). Status: {state}")
        else:
            rospy.logwarn(f"Failed to reach target point (x={x}, y={y}). Status: {state}")

    def feedback_callback(self, feedback):
        # 打印当前机器人与目标的距离差距
        # rospy.loginfo("Moving towards the goal...")
        rospy.loginfo(f"Current x:{feedback.base_position.pose.position.x}, y:{feedback.base_position.pose.position.y}")
