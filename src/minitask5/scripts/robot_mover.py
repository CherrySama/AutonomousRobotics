...
class RobotMover:
    ...
    def send_goal(self, waypoint):
        ...
        # 如果有未完成的目标，取消当前目标
        # if self.topics_manager.move_client.get_state() in [GoalStatus.ACTIVE, GoalStatus.PENDING]:
        #     rospy.logwarn(
        #         "A goal is still active. Cancelling it before sending a new one.")
    ...
    def move(self):
        ...
        # rospy.loginfo(f"index:{self.current_goal_index}")
        ...
        #         self.topics_manager.log_position()
        #     else:
        #         rospy.logwarn(
        #             f"Skipping waypoint -> X: {waypoint.x}, Y: {waypoint.y}")
