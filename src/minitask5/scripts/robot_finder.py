...
class RobotFinder:
    ...
    def show_text_in_rviz(self, text):
        # rospy.loginfo(f"Publishing text in RViz: {text}")
        marker = Marker(
            ...
            color = ColorRGBA(0.0, 1.0, 0.0, 0.8) if self.aim_colour == "green" else ColorRGBA(1.0, 0.0, 0.0, 0.8),
            text=text
        )
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # self.show_text_in_rviz("green box")
    ...
    def depth_callback(self, data):
        # rospy.loginfo("Received depth data")
        try:
            # 将ROS深度图像消息转换为OpenCV图像
            self.depth_image = self.bridge.imgmsg_to_cv2(data, "32FC1")
            # rospy.loginfo("Depth image processed successfully")
        except cv2.cv_bridge.CvBridgeError as e:
    ...
    def is_start_find(self):
        ...
        # if self.object_detected and self.area > self.start_area and self.topics_manager.start_countdown != None and rospy.Time.now() - self.topics_manager.start_countdown > self.topics_manager.max_countdown:
        ...
        if self.object_detected and self.area > start_area and rospy.Time.now() - self.topics_manager.start_countdown > self.topics_manager.max_countdown:
            # self.topics_manager.start_countdown = None
            # rospy.loginfo(f"Start area of {self.aim_colour}:{self.area}")
            return True
        else:
            return False
    ...
    def move_to_target(self):
        ...
        while not rospy.is_shutdown() and self.stop_find:
            ...
            if self.object_detected:
                ...
                # rospy.loginfo(f"area:{self.area}, stop:{self.stop_area}")
    ...
    #     # 将地图数据转化为 numpy 数组
    #     costmap = np.array(data.data).reshape((height, width))

    #     # 将障碍物标记为 1，其他地方为 0
    #     binary_map = (costmap == 100).astype(np.int8)

    #     # 使用连通域分析，找到障碍物簇
    #     labeled_map, num_features = label(binary_map)

    #     small_obstacles = []  # 存储小障碍物的索引
    #     for i in range(1, num_features + 1):
    #         # 获取每个障碍物簇的大小
    #         cluster_size = np.sum(labeled_map == i)

    #         # 判断是否为小障碍物（例如，簇大小小于 20 ）
    #         if 1 <= cluster_size <= 5:  # 设置簇大小的范围
    #             small_obstacles.append(i)

    #     if small_obstacles:
    #         rospy.loginfo(f"Obstacles number: {len(small_obstacles)}")
    #         for obstacle_id in small_obstacles:
    #             obstacle_coords = np.argwhere(labeled_map == obstacle_id)
    #             rospy.loginfo(f"Position of small obstacle: {obstacle_coords.mean(axis=0)}")  # 输出中心点坐标
    #     else:
    #         rospy.loginfo("Not detected obstacles.")
