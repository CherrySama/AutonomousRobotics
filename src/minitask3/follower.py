...
class GreenObjectFollower:
    ...
    def laser_callback(self, data):
        ...
        self.front_ranges = self.range[360-self.front_dect_ang:] + self.range[:self.front_dect_ang]
    ...
    def image_callback(self, data):
        ...
        # 调用图像处理函数来检测绿色物体
        (h, w) = image.shape[:2]
        self.object_detected, self.target_offset, self.centroid, self.area = self.process_image(image)
        self.previous_centroid = self.centroid
        image_resized = cv2.resize(image, (int(w/self.img_size_percent), int(h/self.img_size_percent)))
        colour_resized = cv2.resize(colour_only_img, (int(w/self.img_size_percent), int(h/self.img_size_percent)))
            ...
            if self.state == "FOLLOW_TARGET":
                if self.object_detected:
                    if not self.Obstacle_detect() and self.find_obstacle == False:
                        ...
                        rospy.loginfo("Obstacle detected, switching to AVOID_OBSTACLE state.")
                else:
                    # 如果没有检测到绿色物体，使用上一个记录的位置
                    if self.previous_centroid != (None, None):
                        rospy.loginfo("Using previous target position...")
                        self.twist.angular.z = -float(self.previous_centroid[0] - self.img_size_percent * 4) / 1000
                ...
                if self.front_dis > self.stop_distance:
                    ...
                    rospy.loginfo("Obstacle cleared, switching to FOLLOW_TARGET state.")
                else:
                    # 继续避障
